...
function backToNews(event){
    window.open(window.location.origin+"/technologynews/technology?technology="+document.getElementById("technology_Val").textContent, "_self");
...
for (var i = 1; i <= b.total_levels; i++) { 
    ...
    function gotoTechnology(pg){
        ...
        window.open(window.location.origin+"/technologiesnews/technology?technology="+pg, "_self");
        /*
        $.ajax({
            url         : "hi", // the url where we want to POST
