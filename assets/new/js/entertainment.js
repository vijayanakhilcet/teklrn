    ...
    $.ajax({               
        ...
        // using the done promise callback
         .done(function(data) { 
            ...
            aHtml += '<div class="col-md-6" onclick="lvlclk(\''+data[index].link+'\', \''+data[index].title+'\', \''+data[index].description+'\')"><p style="pointer-events: none; object-fit: cover;"><img style="pointer-events: none; object-fit: cover;" src="'+data[index].link+'" width="100%" height="30px" alt="Image" class="img-fluid"></p><div class="d-flex post-entry"><div class="post-content"><div style="text-transform: capitalize;font-weight: 300 !important; font-family: \'Poppins\', sans-serif; font-size: 16px !important;  color:black !important"><b style=" background: #1c8ccd; color: white; font-size: small; padding-left:2% !important; padding-right:2% !important; padding: .3%; margin-right: 2%; ">NEWS '+(index+1)+' </b>'+data[index].heading+'<img src="/static/image/images/read_b.png" style="width: 20%;float: right;"></div><hr><p style="font-size:13px;display: block; text-overflow: ellipsis;  word-wrap: break-word;  overflow: hidden;  max-height: 3.6em;  line-height: 1.8em;">'+data[index].description+'</p><div class="post-meta">'+a1Html+'</div><div class="post-meta"><span>'+ a2Html+'</span></div></div></div></div>';     
            });
            elm.innerHTML=aHtml;
...
for (var i = 1; i <= b.total_levels; i++) { 
    ...
    function refineSearchView(pg){
        ...
        var html_message ="";
        $.ajax({
            url         : "/getEntertainmentNewsMatchingTheSearchFiltered", // the url where we want to POST
            ...
            .done(function(data) {
                elm.innerHTML="";  
                $.each(data, function(index) {
                      html_message +='<div style="position:relative;" onclick="gotoTechnology(\''+data[index].name+'\')" ><img style="float: left;width: 200px; height:200px;object-fit: cover;" src="'+data[index].imageLink+'" /><p style="padding-left:2%;font-size:small;background-color:white;position:absolute; color:black;bottom: 2px;z-index:31222; width:100%; text-overflow: clip; overflow: hidden; ">'+data[index].name+'</p></div>'
                    });
