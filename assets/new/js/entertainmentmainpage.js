...
for (var i = 1; i <= b.total_levels; i++) { 
    ...
    function gotoTechnology(pg){
        ...
        window.open(window.location.origin+"/entertainment/technology?technology="+pg, "_self");
        /*
        $.ajax({
            url         : "hi", // the url where we want to POST
...
function lvlclk(url, heading, technology){ 
    ...
    return false;   */
    window.open(window.location.origin+"/entertainment/technology/read?link="+url+'?heading='+heading+'?technology='+technology, "_self");     
 }
