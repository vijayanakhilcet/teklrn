...
function designationsClick(){ 
    ...
    function parseVideoLink(linkVid){        
     ...
     function videoClk1(event, crse, lvl, description, videolink, levels_total){ 
      ...
      var iframe = '<video poster="static/image/images/poster.jpg" autoplay muted playsinline id="frameclk" controls controlsList="nodownload"  height="100%" width="100%"><source src="'+parseVideoLink(videolink)+'" type="video/mp4"></video>';
      $.createModalVid({
        message: iframe,
        closeButton:true,
        scrollable:false,
            ...
            $.ajax({               
                ...
                // using the done promise callback
                 .done(function(data) {   
                    ...
                    var aHtml ="";
                    $.each(data, function(index) {
                        aHtml += '<h4  style="border-radius:.3em; background-color: #629DD1; color: white;" href="">'+'<div style="padding:2%; margin-left: 1%;"><a style="float:left; margin-right:1%; color: #e1d7df;">LEVEL '+(index+1)+' </a><div id='+data[index].level+'description>'+data[index].value+ '</div>' +
