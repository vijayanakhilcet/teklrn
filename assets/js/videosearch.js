    ...
    $.ajax({
        ...
        .done(function(data) {
            ...
            $.each(data, function(index) {
                ...
                html_message +='<div style="pointer-events:none !important; visibility:none; padding: 0 !important;" class="w-100pc md-w-33pc p-10"><a href="#" style="padding:0% !important" class="block no-underline p-5 br-8 hover-bg-indigo-lightest-10 hover-scale-up-1 ease-300"><p style=" font-size: medium !important; color: black !important;" class="fw-600 white fs-m3 mt-3">'+'<div style="padding-left: 13px !important;color: white; background-color: #4976c8; font-size: small; padding: 1.2%; border-radius: .5 em;     font-weight: bold;">'+ '['+data[index].description+']  '+'</div><video poster='+data[index].img+' preload="none" style="width: 700px; height: 250px; object-fit: cover;" controls><source src="'+data[index].video+'" type="video/mp4">Your browser does not support the video tag.</video><div style="padding-left: 13px;padding-right: 13px;font-size: small; font-weight: bold;"><!--'+data[index].contentType+'--></div></p></a>';         
                var infos = data[index].news.split(';');
                for (info in infos) {
                    html_message = html_message+'<div class="indigo fs-s3 after-arrow-right my-4" style="text-decoration: underline;padding-left: 13px; !important; font-weight: bold;">'+infos[info]+'</div>';
                }   
