    ...
    $.ajax({
        ...
        .done(function(data) {
            ...
            $.each(data, function(index) {
                ...
                html_message +='<div style="visibility:none; padding: 0 !important;" class="w-100pc md-w-33pc p-10"><a style="padding:0% !important" class="block no-underline p-5 br-8 ease-300"><p style=" font-size: medium !important; color: black !important;" class="fw-600 white fs-m3 mt-3">'+'<div style="padding-left: 13px !important;color: white; background-color: #4976c8; font-size: small; padding: 1.2%; border-radius: .5 em;     font-weight: bold;">'+'TMAIL <i class="fa fa-envelope" aria-hidden="true" style="color:lightyellow; margin-left:1%"></i>  <h style="margin-left:8%;">TMS</h> <i class="fas fa-comment-alt" style="color:lightyellow; margin-left:1%;"></i>'+'  '+'</div><img onclick="playVid1(this)" id=\''+idx+data[index].description.replaceAll(" ", "_")+'_img'+'\' style="width: 700px; height: 300px; object-fit: cover;" class="w-100pc" onerror="this.src=\'/static/image/test/certificate.jpg\'" src='+data[index].img+' /><video id=\''+idx+data[index].description.replaceAll(" ", "_")+'\' onclick="playVid(this)" poster='+data[index].img+' preload="none" style="visibility:hidden; pointer-events: all;width: 0px; height: 0px; object-fit: cover;"><source src="'+data[index].video+'" type="video/mp4">Your browser does not support the video tag.</video><p style="padding-top: 3%;padding-left: 13px;padding-right: 13px;font-weight: 450 !important; text-transform: capitalize;font-size: small !important; color: black !important;">'+data[index].description+'</p></p></a>';         
