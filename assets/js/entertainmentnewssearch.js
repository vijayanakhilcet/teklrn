...
function refineSearchView(pg, lang, idx){
    if(idx==-1){
        ...
        count = 0
        $.ajax({
            url         : "/getEntertainmentMatchingTheSearchNew", // the url where we want to POST
...
function gotoTechnology(pg){
    window.open(window.location.origin+"/entertainmentnews/technology?technology="+pg, "_self");
    /*    $.ajax({
...
function populateStaticNews(pg){
    ...
    $.ajax({
        ...
        data        : {"search_string":pg, "type":"Entertainment"}, // our data object
        dataType    : 'json', // what type of data do we expect back from the server
        encode      : true
    })
        // using the done promise callback
...
function gotoTechnology2(pg, url, img){   
    window.stop();
    window.open(window.location.origin+"/news/technology?technology="+pg+"&Code="+$("#countryCode option:selected").val()+"&direct=Entertainment", "_self");
